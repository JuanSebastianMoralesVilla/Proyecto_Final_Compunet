...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity httpSecurity) throws Exception {
		//httpSecurity.cors().disable().csrf().disable().authorizeRequests().cors().disable().csrf().disable()

		httpSecurity.cors().disable().csrf().disable().authorizeRequests().antMatchers(HttpMethod.GET, "/templatesProduct/Index").hasAnyRole("admin","operador").
